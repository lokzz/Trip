    ...
    if name == "":
        ...
        with open('spots.json', 'w') as json_file:
            try:
                ...
                defaultdict = {"1":{"name": name, "type":"normal", "date": "", "estarrival": "", "location": "", "description": ""}}
...
def spots(request: Request):
    ...
    if (requestedspot+".json") in os.listdir("trips"):
        def saver():
            json.dump(spotjson, open("trips/"+requestedspot+".json", "w"))
            ui.open("spots?name=_"+str(requestedspot))
        def new(num):
            intd = int(num)
            intde = str(intd + 1)
            spotjson.update({intde: {"name": "", "type": "normal", "date": "", "estarrival": "", "location": "", "description": ""}})
            json.dump(spotjson, open("trips/"+requestedspot+".json", "w"))
            ui.open("spots?name=_"+str(requestedspot))
        
        spotjson = json.load(open("trips/"+requestedspot+".json", "r"))
        write_log("[" + requestedspot + "]" +" called")
        with ui.stepper().props('vertical').classes('w-full') as stepper:
            for key, i in spotjson.items():
                with ui.step(str(key)) as e:
                    spotnamev = i["name"]
                    spottypev = i["type"]
                    spotdatev = i["date"]
                    spotarrif = i["estarrival"]
                    spotlocv = i["location"]
                    spotdescv = i["description"]
                    spotname = ui.input(value=str(spotnamev), label="name:") #, on_change=lambda spotname, i=i, s=spotnamev: i.update({"name": spotname.value}))
                    spottype = ui.input(value=str(spottypev), label="type:") #, on_change=lambda spottype, i=i: i.update({"type": spottype.value}))
                    spotdate = ui.input(value=str(spotdatev), label="date:") #, on_change=lambda spotdate, i=i: i.update({"date": spotdate.value}))
                    spotarriv = ui.input(value=str(spotarrif), label="arrive at:") #, on_change=lambda spotarriv, i=i: i.update({"estarrival": spotarriv.value}))
                    spotloc = ui.input(value=str(spotlocv), label="location:") #, on_change=lambda spotloc, i=i: i.update({"location": spotloc.value}))
                    spotdesc = ui.input(value=str(spotdescv), label="description:") #, on_change=lambda spotdesc, i=i: i.update({"description": spotdesc.value}))
                    def stapernext():
                        spotnamev = i["name"]
                        spottypev = i["type"]
                        spotdatev = i["date"]
                    ...
                    with ui.stepper_navigation():
                        ui.button("Next", on_click=lambda: stapernext())
                        ui.button("Back", on_click=lambda: staperback())
            with ui.row():
                ui.button("Save", on_click=lambda: saver())
                ui.button("Add (will save)", on_click=lambda: new(key))
    else:
